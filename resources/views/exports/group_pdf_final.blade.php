...
<head>
    ...
    <style>
        ...
        @page {
            /* กำหนดระยะขอบบนให้พอดีกับความสูงของ Header เพื่อไม่ให้เนื้อหา (Table) ไปทับหัวกระดาษ */
            margin-top: 80px;
            margin-bottom: 50px;
        }
        ...
        .page-header {
            position: fixed;
            top: -60px;
            /* ขยับขึ้นไปอยู่ในส่วนของ margin-top */
            left: 0px;
            right: 0px;
            height: 50px;
            text-align: center;
...
<body>
    ...
    <div class="page-header">
        <table style="width: 100%;">
            <tr>
                <td align="center">
                    <b style="font-size: 14pt;">สรุปรายชื่อพนักงาน เบิกค่าเดินทาง/เบี้ยเลี้ยง บริษัท
                        {{ $exgroup->plantname }}</b>
                </td>
            </tr>
            <tr>
            ...
            <tbody>
                ...
                <tr style="background-color:#C9DAF8; font-weight: bold;">
                    ...
                    <td style="text-align: right; text-decoration: underline;">
                        {{ number_format(round($grand_food), 2) }}
                    </td>
                    <td style="text-align: right; text-decoration: underline;">
                        {{ number_format(round($grand_gas), 2) }}
                    </td>
                    <td style="text-align: right; text-decoration: underline;">
                        {{ number_format(round($grand_express), 2) }}</td>
                    <td style="text-align: right; text-decoration: underline;">
                        {{ number_format(round($grand_public), 2) }}
                    </td>
                    <td style="text-align: right; text-decoration: underline;">
                        {{ number_format(round($grand_other), 2) }}
                    </td>
                    <td style="text-align: right; text-decoration: underline;">
                        {{ number_format(round($grand_total), 2) }}
                    </td>
                </tr>
            </tbody>
        </table>
                ...
                <tr style="height: 40px;">
                    ...
                    <td colspan="2" style="border: 1px solid #000; text-align: center;">
                        @if (!empty($signatures['nextapproved']) && file_exists(public_path("storage/{$signatures['nextapproved']}")))
                            <img src="{{ public_path("storage/{$signatures['nextapproved']}") }}"
                                style="height: 40px;">
                        @endif
                    </td>
                    <td colspan="2" style="border: 1px solid #000; text-align: center;">
